...
class CloudSpannerInstanceDeployOperator(BaseOperator):
    ...
    template_fields = ('project_id', 'instance_id', 'configuration_name', 'node_count',
    ...
    def execute(self, context):
        if not self._hook.get_instance(self.project_id, self.instance_id):
            self.log.info("Creating Cloud Spanner instance '{}'".format(self.instance_id))
            func = self._hook.create_instance
        else:
            self.log.info("Updating Cloud Spanner instance '{}'".format(self.instance_id))
...
class CloudSpannerInstanceDeleteOperator(BaseOperator):
    ...
    def execute(self, context):
        if self._hook.get_instance(self.project_id, self.instance_id):
            ...
            print("Instance '{}' does not exist in project '{}'. Aborting delete."
...
class CloudSpannerInstanceDatabaseQueryOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        self.log.info("Executing DML query(-ies) on projects/{}/instances/{}/databases/{}"
                      .format(self.project_id, self.instance_id, self.database_id))

...
INSTANCE_NAME = os.environ.get('INSTANCE_NAME', 'test-postgres')
INSTANCE_NAME2 = os.environ.get('INSTANCE_NAME2', 'test-mysql')
...
with models.DAG(
    ...
    sql_gcp_add_bucket_permission = GoogleCloudStorageBucketCreateAclEntryOperator(
        entity="user-{{ task_instance.xcom_pull('sql_instance_create', key='return_value') }}",
        role="WRITER",
    ...
    sql_gcp_add_object_permission = GoogleCloudStorageObjectCreateAclEntryOperator(
        entity="user-{{ task_instance.xcom_pull('sql_instance_create_2', key='return_value') }}",
        role="READER",

...
SQL = [
    ...
    'CREATE TABLE IF NOT EXISTS TABLE_TEST (I INTEGER)',  # shows warnings printed out
...
os.environ['AIRFLOW_CONN_PROXY_POSTGRES_TCP'] = \
    "cloudsql://{user}:{password}@{public_ip}:{public_port}/{database}?" \
...
os.environ['AIRFLOW_CONN_PROXY_MYSQL_SOCKET'] = \
    ...
    "sql_proxy_use_tcp=False".format(
        ...
        instance=quote_plus(MYSQL_INSTANCE_NAME),
        database=quote_plus(MYSQL_DATABASE_NAME),
        sql_proxy_binary_path=quote_plus(sql_proxy_binary_path))
...
os.environ['AIRFLOW_CONN_PUBLIC_MYSQL_TCP_SSL'] = \
    ...
    "sslcert={client_cert_file}&" \
    "sslkey={client_key_file}&" \
    "sslrootcert={server_ca_file}".format(
        user=quote_plus(MYSQL_USER),
        password=quote_plus(MYSQL_PASSWORD),
        public_port=MYSQL_PUBLIC_PORT,
        public_ip=quote_plus(MYSQL_PUBLIC_IP),
...
with models.DAG(
    ...
    sql_instance_deploy_task_proxy_postgres_tcp = CloudSqlQueryOperator(
        cloudsql_conn_id='proxy_postgres_tcp',
        project_id=PROJECT_ID,
        task_id="example_gcp_sql_task_proxy_postgres_tcp",
        sql=SQL
    )
    sql_instance_deploy_task_proxy_postgres_socket = CloudSqlQueryOperator(
        cloudsql_conn_id='proxy_postgres_socket',
        project_id=PROJECT_ID,
        task_id="example_gcp_sql_task_proxy_postgres_socket",
        sql=SQL
    )
    sql_instance_deploy_task_public_postgres_tcp = CloudSqlQueryOperator(
        cloudsql_conn_id='public_postgres_tcp',
        project_id=PROJECT_ID,
        task_id="example_gcp_sql_task_public_postgres_tcp",
        sql=SQL
    )
    sql_instance_deploy_task_public_postgres_tcp_ssl = CloudSqlQueryOperator(
        cloudsql_conn_id='public_postgres_tcp_ssl',
        project_id=PROJECT_ID,
        task_id="example_gcp_sql_task_public_postgres_tcp_ssl",
        sql=SQL
    )
    sql_instance_deploy_task_proxy_mysql_tcp = CloudSqlQueryOperator(
        cloudsql_conn_id='proxy_mysql_tcp',
        project_id=PROJECT_ID,
        task_id="example_gcp_sql_task_proxy_mysql_tcp",
        sql=SQL
    )
    sql_instance_deploy_task_proxy_mysql_socket = CloudSqlQueryOperator(
        cloudsql_conn_id='proxy_mysql_socket',
        project_id=PROJECT_ID,
        task_id="example_gcp_sql_task_proxy_mysql_socket",
        sql=SQL
    )
    sql_instance_deploy_task_public_mysql_tcp = CloudSqlQueryOperator(
        cloudsql_conn_id='public_mysql_tcp',
        project_id=PROJECT_ID,
        task_id="example_gcp_sql_task_public_mysql_tcp",
        sql=SQL
    )
    sql_instance_deploy_task_public_mysql_tcp_ssl = CloudSqlQueryOperator(
        cloudsql_conn_id='public_mysql_tcp_ssl',
        project_id=PROJECT_ID,
        task_id="example_gcp_sql_task_public_mysql_tcp_ssl",
        sql=SQL
    )

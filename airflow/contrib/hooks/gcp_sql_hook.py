...
class CloudSqlProxyRunner(LoggingMixin):
    ...
    def _download_sql_proxy_if_needed(self):
        ...
        if r.status_code != 200:
            raise AirflowException("The cloud-sql-proxy could not be downloaded. "
...
class CloudSqlDatabaseHook(BaseHook):
    ...
    The hook is a "meta" hook - it does not perform an actual connectivity,
    it is there to retrieve all the parameters configured in cloudsql:// connection,
    start/stop Cloud Sql Proxy if needed, dynamically generate Postgres or MySQL
    connection in the Connection database and return an actual Postgres or MySQL hook
    hook to perform the connectivity. The returned Postgres/MySQL hooks are using direct
    connection or Cloud Sql Proxy as configured)


    Main parameters of the hook are retrieved from the URI components:
    ...
    * sql_proxy_use_tcp - (default False) If set to true, TCP is used to connect via proxy.
      otherwise UNIX sockets are used.
    ...
    * sslrootcert - Path to server's certificate to authenticate when SSL is used.
    ...
    def __init__(self, cloudsql_conn_id='google_cloud_sql_default'):
        ...
        self.cloudsql_conn_id = cloudsql_conn_id
        self.cloudsql_connection = self.get_connection(self.cloudsql_conn_id)
    ...
    def _generate_unique_path(self):
        # Note we are not using mkdtemp here as the path generated with mkdtemp
        # can be close to 60 characters and there is a limitation in
        # length of socket path to around 100 characters in total - we are
        # appending project/location/instance to it later on and postgres
        # appends its own prefix, so we chose a shorter "/tmp/{uuid1}" - based
        # on host name and clock + clock sequence, which should be fairly
        # sufficient for our needs and should even work if the time is set back
        # we are using db_conn_id so that we can easily match socket path
        # and DB connection created dynamically

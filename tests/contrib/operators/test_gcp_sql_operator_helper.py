...
from tests.contrib.operators.test_gcp_base_system_test import RetrieveVariables
...
class CloudSqlQueryTestHelper(LoggingCommandExecutor):
    ...
    def raise_database_exception(self, database):
        raise Exception("The {database} instance does not exist. Make sure to run  "
                        "`python {f} --action=create-query` before running the test"
                        " (and remember to run `python {f} --action=delete-query` "
                        "after you are done."
...
if __name__ == '__main__':
    ...
    helper.info('Starting action: {}'.format(action))
    ...
    helper.info('Finishing action: {}'.format(action))

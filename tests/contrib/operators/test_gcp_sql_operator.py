...
class CloudSqlQueryValidationTest(unittest.TestCase):
    ...
    def test_create_operator_with_wrong_parameters(self,
        ...
        connection.parse_from_uri(
            "cloudsql://user:password@8.8.8.8:3200/testdb?database_type={database_type}&"
    ...
    def test_create_operator_with_correct_parameters_postgres(self, get_connections):
        connection = Connection()
        connection.parse_from_uri(
            "cloudsql://user:password@8.8.8.8:3200/testdb?database_type=postgres&"
    ...
    def test_create_operator_with_correct_parameters_mysql(self, get_connections):
        connection = Connection()
        connection.parse_from_uri(
            "cloudsql://user:password@8.8.8.8:3200/testdb?database_type=mysql&"
            "project_id=example-project&location=europe-west1&instance=testdb&"
            "use_proxy=False&use_ssl=False")
        get_connections.return_value = [connection]
        operator = CloudSqlQueryOperator(
...
class CloudSqlProxyTest(BaseGcpIntegrationTestCase):
    def __init__(self, method_name='runTest'):
        super(CloudSqlProxyTest, self).__init__(
            method_name,
    ...
    def test_start_proxy_fail_no_parameters(self):
        ...
        with self.assertRaises(AirflowException) as cm:
            runner.start_proxy()
        err = cm.exception
        self.assertIn("invalid instance name", err.message)
        with self.assertRaises(AirflowException) as cm:
            runner.start_proxy()
        err = cm.exception
        self.assertIn("invalid instance name", err.message)
...
@unittest.skipIf(
    BaseGcpIntegrationTestCase.skip_check(GCP_CLOUDSQL_KEY), SKIP_TEST_WARNING)
class CloudSqlExampleDagsTest(BaseGcpIntegrationTestCase):
    def __init__(self, method_name='runTest'):
        super(CloudSqlExampleDagsTest, self).__init__(
            method_name,
            dag_id='example_gcp_sql_query',

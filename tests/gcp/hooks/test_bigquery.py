...
class TestBigQueryBaseCursor(unittest.TestCase):
    ...
    def test_validate_src_fmt_configs(self):
        ...
        self.assertIn("test_config_known", src_fmt_configs)
        self.assertIn("compatibility_val", src_fmt_configs)
...
class TestBigQueryHookLegacySql(unittest.TestCase):
    ...
    @mock.patch("airflow.gcp.hooks.bigquery.BigQueryBaseCursor.run_with_configuration")
    def test_hook_uses_legacy_sql_by_default(self, run_with_config, mock_get_creds_and_proj_id):
        with mock.patch("airflow.gcp.hooks.bigquery.BigQueryHook.get_service"):
            bq_hook = hook.BigQueryHook()
    ...
    @mock.patch("airflow.gcp.hooks.bigquery.BigQueryBaseCursor.run_with_configuration")
    def test_legacy_sql_override_propagates_properly(self, run_with_config, mock_get_creds_and_proj_id):
        with mock.patch("airflow.gcp.hooks.bigquery.BigQueryHook.get_service"):
...
class TestBigQueryHookLocation(unittest.TestCase):
    ...
    @mock.patch("airflow.gcp.hooks.bigquery.BigQueryBaseCursor.run_with_configuration")
    def test_location_propagates_properly(self, run_with_config, mock_get_creds_and_proj_id):
        with mock.patch("airflow.gcp.hooks.bigquery.BigQueryHook.get_service"):
            ...
            self.assertEqual(run_with_config.call_count, 1)
